...
/* =========================================================
   AquaSense • ESP32 Climate Controller (Demo IoT)
   - Monitorização: Temperatura/Humidade (DHT11)
   - Atuação: PWM em carga 12V via MOSFET (low-side switching)
   - Interface: Web UI + API JSON (arquitetura típica IoT)
   ========================================================= */
...
// Hardware (atuador)
// ---------------------
const int pinmosfet = 18;      // Gate do MOSFET (PWM)
const int freq = 5000;         // Frequência PWM (Hz)
const int resolution = 8;      // Resolução PWM (bits) → 0..255
...
// =========================================================
// Estado do sistema (runtime)
// =========================================================
volatile bool modeAuto = true; // AUTO: decisão pelo sensor | MANUAL: brilho fixo

float tempC = NAN;            // Temperatura (°C)
float humP  = NAN;            // Humidade (%)

unsigned long lastSensorRead = 0;
const unsigned long SENSOR_INTERVAL_MS = 2000; // DHT11 é lento → intervalo mínimo recomendado

// Limiares do controlo (AUTO)
float T_LOW  = 24.0;          // Abaixo → LEDs OFF
float T_HIGH = 28.0;          // Acima → LEDs MAX
float HYST   = 0.3;           // Histerese: evita liga/desliga constante perto do limiar

// Controlo manual
uint8_t manualPWM  = 120;     // Valor PWM em modo MANUAL (0..255)

// Estado do atuador
uint8_t currentPWM = 0;       // PWM aplicado atualmente
bool forcedOn = false;        // Memória de estado para histerese (estabilidade)
...
/* Aplica PWM ao MOSFET.
   Nota: ESP32 core 3.x usa ledcAttach(pin,...) e ledcWrite(pin, duty). */
void applyPWM(uint8_t pwm) {
  currentPWM = pwm;
  ledcWrite(pinmosfet, currentPWM);
}

/* Leitura do sensor com cache (evita leituras repetidas e falhas).
   Mantém o último valor válido para estabilidade da UI/controlo. */
...
/* Lógica de controlo:
   - MANUAL: aplica manualPWM.
   - AUTO: OFF abaixo de T_LOW, MAX acima de T_HIGH,
           proporcional entre limites, com histerese para evitar flapping. */
void controlLogic() {
  ...
  // Segurança: se não houver leitura válida, desliga (fail-safe)
  if (isnan(tempC)) {
    applyPWM(0);
    forcedOn = false;
    return;
  ...
  // Histerese: mantém estado ON até descer suficientemente; e vice-versa
  if (forcedOn) {
    if (tempC <= (T_LOW - HYST)) {
      forcedOn = false;
      applyPWM(0);
  ...
  // Controlo proporcional na zona intermédia
  if (tempC <= T_LOW) {
    applyPWM(0);
...
// =========================================================
// Estado para UI (labels + mensagens)
// =========================================================
String tempZone() {
...
String globalState(String &msg){
  if (isnan(tempC) || isnan(humP)) {
    msg = "Sensor a estabilizar / leitura falhou (DHT11 é lento).";
    return "AVISO";
  }
  if (tempC >= (T_HIGH + 1.5)) {
    msg = "Temperatura elevada. Sistema a compensar com brilho alto.";
    return "ALERTA";
  }
...
/* API JSON: expõe o estado atual para a UI.
   Mantém a estrutura simples para integração fácil com dashboards externos. */
String buildStatusJson(){
  String msg;
  String state = globalState(msg);
...
<head>
  ...
  <style>
    :root{
      --bg:#0b1220; --txt:#e8eefc; --muted:#9fb0d0; --line:rgba(255,255,255,.08);
...
<div class="wrap">
  ...
  <div class="grid">
    <section class="card">
      <div class="hd"><strong>Dashboard</strong><span class="muted">Atualiza a cada 2s</span></div>
      <div class="bd">
        <div class="kpis">
          <div class="kpi">
...
void setup() {
  ...
  // Inicialização PWM (ESP32 core 3.x)
  ...
  server.on("/api/config", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    // Validação mínima para evitar configuração inválida
...
void loop() {
  // Loop leve e não-bloqueante: controlo contínuo e estável
  controlLogic();
  delay(5);
}